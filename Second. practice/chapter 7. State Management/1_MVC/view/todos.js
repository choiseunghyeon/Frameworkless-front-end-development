...
const attachEventsToTodoElement = (element, index, events) => {
  ...
  element.querySelector("input.toggle").addEventListener("click", e => events.toggleItemCompleted(index));
...
export default (targetElement, state, events) => {
  ...
  filteredTodos.map((todo, index) => getTodoElement(todo, index, events)).forEach(element => newTodoList.appendChild(element));
